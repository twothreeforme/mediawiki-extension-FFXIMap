...
(function (factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    // AMD
    // eslint-disable-next-line
    define(['ext.leafletMain'], factory)
  } else if (typeof module !== 'undefined') {
    // Node/CommonJS
    module.exports = factory(require('ext.leafletMain'))
  } else {
    // Browser globals
  ...
  L.Control.Search = L.Control.extend({
    ...
    /* need to prioritize records cache */
    _fillRecordsCache: function () {
      ...
      if (this.options.layer) {
        ...
        if (this.options.sourceData) { this._recordsCache = this.options.sourceData } 
        ...
        let records = [];
        for (const [key, value] of Object.entries(this._recordsCache)){
          let search = value.toLowerCase().search(this._input.value.toLowerCase());
          if (search > -1) {
            //console.log(value);
            records[key] = value;
          }
    ...
    _handleSubmit: function () { // button and tooltip click and enter submit
      ...
      if (this._input.style.display === 'none') { // on first click show _input only
        ...
        if (this._input.value === '') { // hide _input only
          ...
          const mapFound = this._getMap(this._input.value)
          
          if (!mapFound) {
            this.showAlert()
          } else {
            this.fire('search:locationfound', {
              mapid: this._input.value
            })
            this.collapse()
          }
        }
  ...
  /*  REMOVE EVENTUALLY - creates red circle around selected search item
  */
